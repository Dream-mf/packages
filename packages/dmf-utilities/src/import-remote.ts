	...
	new Promise<void>((resolve, reject) => {
		...
		__webpack_require__.l(
			...
			(event) => {
				...
				DreamMFLogClient.logException({ url, scope, timestamp, enableNoCache }, error);
				reject(error);
			},
			scope
		);
...
const importRemote = async <T>({
	...
	if (!window[scope]) {
		...
		DreamMFLogClient.logGeneral({ message: 'Loaded a remote through module federation', url: remoteUrl, scope, module, remoteUrlFallback, enableNoCache });
		if (!window[scope]) {
			const error = new Error(
				`Remote loaded successfully but ${scope} could not be found! Verify that the name is correct in the Webpack configuration!`
			);
			console.log({ error });
			DreamMFLogClient.logException({ url: remoteUrl, scope, module, remoteUrlFallback, enableNoCache }, error);
			throw error;
		}
