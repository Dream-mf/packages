...
export async function addInterceptors(config: DreamMFAuthConfig) {
	...
	DreamMFLogClient.logAuthentication({ message: `Appending fetch interceptor.` });
	const { fetch: originalFetch } = window;
	window.fetch = async (...args) => {
		const [input, init] = args;
		if (!init) {
			DreamMFLogClient.logFetch({ url: input, user: null });
		...
		if (useAuth) {
			...
			DreamMFLogClient.logFetch({ url: input, id: id, user: user.profile });
			if (user) {
				fetchConfig.headers = {
					'X-Correlation-Id': id,
